...
<summary>Project mockup</summary>

![project mockup](https://media.git.generalassemb.ly/user/17300/files/d88b5a00-3da9-11ea-9d57-7a742c153636
...
3.  [Create files for components and empty App](#create-files-for-components-and-empty-app)
4.  [Create barebones SearchForm and SearchResults components](#create-barebones-searchform-and-searchresults-components)
5.  [Add and style the SearchForm component JSX](#add-and-style-searchform-component-jsx)
6.  [Add and style SearchResults component JSX](#add-and-style-searchresults-component-jsx)
7.  [Store your API key safely](#store-your-api-key-safely)
8.  [Add the Giphy API request](#add-the-giphy-api-request)
9.  [Add SearchForm functionality](#add-searchform-functionality)
10. [Improving the User Experience](#improving-the-user-experience)
11. [Working with SVGs and addressing a11y](#working-with-svgs-and-addressing-a11y)
...
2.  Set up an app by clicking the Create App in the navbar to obtain your beta key.

## 2. Create files for components and empty App
...
## 3. Create barebones SearchForm and SearchResults components
...
Once you've completed both components, import them into App.js and add them into the JSX below the `<h1>` element. You should now see the titles for SearchForm and SearchResults displaying.
...
## 4. Add and style the SearchForm component JSX

Replace the placeholder content in the SearchForm component with a form. We're not going to worry about the functionality yet. We're just going to get our form styled to look like the project's mock up.
...
In the `src/index.css` file delete the styles and add the following styles. Check the result in the browser and make sure that the form with the input and button display properly before moving on to the next step.
...
## 5. Add and style SearchResults component JSX
...
If there are no images, we'll just return an `h2` that reads: No Images Found!. Remember that the code inside an if statement block only runs if the condition in the parentheses evaluates to true. So, if it's true that the images array has no length, then the return statement runs and the rest of the function is ignored. If the condition is not true — meaning the images array does have a length — it skips the code block and runs the return to display the search results.
...
Create a new file in the project root directory (in the outer project folder where the README.md file is **not** in the `src` directory) called `.env.local` . Yes, this file's name starts with a dot (`.` )! Make sure you name it ***exactly*** as you see it here.
...
When your app loads environment variables like the `REACT_APP_GIPHY_KEY` variable that we just created are loaded into memory. This means that *if your server is already running* , you need to stop it and restart it after adding or changing an environment variable, so do that now.

[](#add-the-giphy-api-request)

## 7. Add the Giphy API request
...
The getImages function needs to construct a request URL from the options; make the request to the Giphy API; and, if successful, set the response in the images array in state. We want to test things out before we connect our search form, so we'll just create a `searchString` variable inside of getImages to hold a string so we can simulate the results.
...
Cool, we're almost there... we just need to change the image src in SearchResults component to display the image from `image.images.downsized_large.url` now.
...
## 8. Add SearchForm functionality
...
Lastly, we need to wire up our form. In the SearchForm component, add an `onSubmit` attribute to the form element and set it to the handleSubmit function we passed to the SearchForm as a prop. Similarly, in the input element, add an `onChange` attribute and set it to the handleChange function that is received as a prop. All that's left is to add the `value` attribute to the input and set it to the `searchString` prop.
...
ive a searchString as an argument. This is arguably better than just leaving it to use the value that is currently in state when the function runs. It also means that we can safely use the searchString as the value we pass to  `setLastSearch`because we don't need to worry about the asynchronous way in which state is updated.
...
## 10. Working with SVGs and addressing a11y
...
-   Use React Router to allow the user to see a *details* view for each gif like the [Giphy.com](http://giphy.com/) site.

Copyright © General Assembly 2021